...
export default function RoomsPage() {
    ...
    return (
        <div className="p-8">
            <h1 className="text-3xl font-bold mb-6" style={{color: '#7D3837'}}>Chambres</h1>
            
            {showForm && (
                <div className="bg-yellow-50 border rounded p-4 mb-4" style={{borderColor: '#7D3837'}}>
                    <h3 className="font-bold mb-2" style={{color: '#7D3837'}}>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200">
                <div className="p-6 border-b border-slate-200">
                    <div className="flex items-center justify-between">
                        <h2 className="text-xl font-semibold text-slate-800">Liste des Chambres</h2>
                ...
                <div className="p-6">
                    {rooms.length === 0 ? (
                        <div className="text-center py-12">
                            ...
                            <p className="text-slate-500 text-lg font-medium">Aucune chambre enregistrée</p>
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                                            ...
                                            {(user?.role === 'admin' || user?.role === 'super_admin') && (
                                                <div 
                                                    ...
                                                    className={`${categoryStyle.bgColor} p-5 rounded-xl border-2 border-dashed border-slate-300 hover:border-slate-400 hover:shadow-lg transition-all duration-200 cursor-pointer group flex flex-col items-center justify-center min-h-[140px]`}
                                                >
                                                    <div className={`w-10 h-10 bg-gradient-to-br ${categoryStyle.color} rounded-lg flex items-center justify-center mb-3 group-hover:scale-110 transition-transform`}>
                                                        <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                                                        </svg>
                                                    </div>
                                                    <p className="text-sm font-medium text-slate-600 text-center">Ajouter une chambre</p>
                                            ...
                                            {categoryRooms.map(room => {
                                                return (
                                                    <div key={room.id} className={`${categoryStyle.bgColor} p-5 rounded-xl border border-slate-200 hover:shadow-lg transition-all duration-200 hover:border-slate-300 ${(user?.role === 'admin' || user?.role === 'super_admin') ? 'cursor-pointer' : ''} group`} onClick={(user?.role === 'admin' || user?.role === 'super_admin') ? () => handleEditRoom(room) : undefined}>
                                                        ...
                                                        <h4 className="font-semibold text-slate-800 text-lg mb-2">Chambre {room.number}</h4>
                                                        <p className={`text-xs font-medium mb-3 ${categoryStyle.textColor}`}>{room.category}</p>
                                                        
                                                        <div className="flex items-center justify-between">
                                                            <span className="text-sm text-slate-600">Prix/nuit</span>
                                                            <span className={`text-lg font-bold bg-gradient-to-r ${categoryStyle.color} bg-clip-text text-transparent`}>{formatPrice(room.price)}</span>
                                                        </div>
                                                    </div>
                                                );
                                            })}
